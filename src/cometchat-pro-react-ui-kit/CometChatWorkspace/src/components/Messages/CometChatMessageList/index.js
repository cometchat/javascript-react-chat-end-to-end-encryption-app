...
class CometChatMessageList extends React.PureComponent {
  ...
  fetchMessages = () => {
    // Step 4 of 14
    // The user can click on any of the conversation from the Conversation list.
    // Hence, decrypt the older messages fetched for that conversation.
    // Refer to CometChatMessageComposer/index.js for Step 5.
    const decryptMessage = async (messageList) => {
      messageList = await messageList.map(async (message) => {
        try {
          const text = message?.data?.text;
          const sender = message.sender.uid;
          const receiver = message.receiverId;
          const receiverType = message.receiverType;
          if (text && sender && receiver && receiverType) {
      ...
      return await Promise.all(messageList);
    };
    const promise = new Promise((resolve, reject) => {
      this.MessageListManager.fetchPreviousMessages()
        .then(async (messageList) => {
          messageList = await decryptMessage(messageList);
          resolve(messageList);
        })
