...
import { CometChatCreatePoll, CometChatStickerKeyboard } from '../Extensions';
...
class CometChatMessageComposer extends React.PureComponent {
  ...
  sendTextMessage = async () => {
    ...
    // Step 5 of 14
    // Once the previous conversation has been loaded and decrypted,
    // the user can now choose to send a message as a response.
    // Hence, encrypt the messageInput and return the CometChat.TextMessage object
    // with the encrypted messageInput.
    // Scroll down for step 6 to learn about sending this message.
    const getEncryptedCometChatTextMessage = async (
      receiverId,
      messageInput,
      receiverType
    ) => {
      const encryptedText = await window.CometChatVirgilHelper.encryptText({
        text: messageInput,
        receiver: receiverId,
        receiverType,
      });
      ...
      textMessage.setSender(this.loggedInUser);
      textMessage.setReceiver(this.context.type);
      textMessage.setText(encryptedText);
      textMessage._composedAt = getUnixTimestamp();
      textMessage._id = ID();
      return textMessage;
    };

    const encryptedTextMessage = await getEncryptedCometChatTextMessage(
    ...
    // Step 6 of 14
    // Send the encrypted text message via CometChat.
    CometChat.sendMessage(encryptedTextMessage)
      .then(async (message) => {
        // Step 7 of 14
        // The original text can be safely shown to the sender.
        // Scroll down for Step 8 to learn about encrypting edited message
        message.data.text = messageInput;
  ...
  editMessage = async () => {
    ...
    // Step 8 of 14
    // The user can choose to edit the sent message.
    // Hence, this shows how to encrypt the edited message again.
    // Scroll down to step 9 to learn about
    const getEncryptedCometChatTextMessage = async (
      receiverId,
      messageText,
      receiverType
    ...
    // Refer to CometChatConversationList/controller.js for step 10
    CometChat.editMessage(encryptedTextMessage)
      .then(async (message) => {
        const encryptedText = message.data.text;
        const receiverType = message.receiverType;
        const sender = message.data.entities.sender.entity.uid;
        const receiver = message.receiverId;

        const decryptText = await window.CometChatVirgilHelper.decryptText({
          encryptedText,
          receiver,
          receiverType,
          sender,
        ...
        message.data.text = decryptText;
        message.text = decryptText;
