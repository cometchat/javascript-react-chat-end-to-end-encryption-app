  ...
  getConversations = () => {
    this.ConversationListManager.fetchNextConversation()
      .then(async (conversationList) => {
        ...
        // Step 3 of 14
        // Conversation list shows the last message for a particular conversation.
        // Hence, decrypt the last message of each conversation.
        // Refer to CometChatMessageList/index.js for step 4
        const conversationLists = conversationList.map(async (conversation) => {
          try {
            if (conversation.lastMessage.type === 'text') {
              const sender = conversation.lastMessage.sender.uid;
              const receiver = conversation.lastMessage.receiverId;
              const encryptedText = conversation.lastMessage.text;
              const receiverType = conversation.lastMessage.receiverType;

              const decryptText =
                await window.CometChatVirgilHelper.decryptText({
                  encryptedText,
                  sender,
                  receiver,
                  receiverType,
                });
              conversation.lastMessage.text = decryptText;
              conversation.lastMessage.data.text = decryptText;
              return conversation;
            } else {
              return conversation;
            }
        ...
        await Promise.all(conversationLists);
